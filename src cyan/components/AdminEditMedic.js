...
savePassword=e=>{
        ...
        if(password == password2){
            ...
            axios.put(`http://127.0.0.1:8000/api/changepw?id=${id}`,patrec).then(res=>console.log(res.data));
    ...
    return (
      <div className="background white">
        ...
        <div class="navbar-fixed">
                   ...
                   <li> <Link to ="/adminmedics" className="btn cyan white-text text-darken-1" ><span className="cyan-text glyphicon glyphicon-globe"></span>Medics Records</Link></li>
...
<li><a onClick={this.props.logout.bind(this)} href="/login" class="cyan-text"><span className="glyphicon glyphicon-log-out"></span> Logout</a></li>
        ...
        <div className="cyan center" style={{marginTop:"-28px",marginLeft:"-13.5px",width:"105%",backgroundColor:"linear-gradient(to left, #80deea  0%,#00bcd4   100%);"}}>
        ...
        <div className="row  ">
                ...
                {this.state.edit?<input type="text" name="name" onChange={this.onChange} value={name}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="name" value={this.props.medicOne.map(data=> data.name)}>
                     </input>}   </div>
                ...
                {this.state.edit?<input type="text" name="birthdate" onChange={this.onChange} value={birthdate}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="birthdate" value={this.props.medicOne.map(data=> data.birthdate)}>
                     </input>} 
                  </div>
                <div className = "col s6"><h5>Sex:</h5>
                {this.state.edit?<input type="text" name="sex" onChange={this.onChange} value={sex}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="sex" value={this.props.medicOne.map(data=> data.sex)}>
                     </input>} 
                  </div>
                ...
                {this.state.edit?<input type="text" name="hospital" onChange={this.onChange} value={hospital}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="birthdate" value={this.props.medicOne.map(data=> data.hospital)}>
                     </input>} 
                  </div>
                <div className = "col s6"><h5>Role:</h5>
                {this.state.edit?<input type="text" name="position" onChange={this.onChange} value={position}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="position" value={this.props.medicOne.map(data=> data.position)}>
                     </input>} 
                  </div>
                ...
                {this.state.edit?<input type="text" name="address" onChange={this.onChange} value={address}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="address" value={this.props.medicOne.map(data=> data.address)}>
                     </input>} 
                  </div>
               <div className = "col s6"><h5>Email:</h5>
               {this.state.edit?<input type="text" name="email" onChange={this.onChange} value={email}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="email" value={this.props.medicOne.map(data=> data.email)}>
                     </input>} 
                 </div>
                <div className = "col s6"><h5>Phone Number:</h5> {this.state.edit?<input type="text" name="phone_number" onChange={this.onChange} value={phone_number}></input>
                 :<input style={{color:"#4fc3f7",fontWeight:"bold"}} type ="text" name="phone_number" value={this.props.medicOne.map(data=> data.phone_number)}>
                     </input>} </div>
