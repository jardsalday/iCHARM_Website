...
export class ListEditPatient extends Component{
  ...
  test120(){
    ...
    axios.post('http://127.0.0.1:8000/api/loglog/',send).then(res=>console.log(res.data));
  }
  test140(){
  ...
  refresh1(){
    this.setState({refresh:!this.state.refresh})
      ...
      const holder ="1";
      const send = {holder};
      axios.post('http://127.0.0.1:8000/api/log/',send).then(res=>console.log(res.data))
    
    // else{
    //   console.log("LOW");
    //   const holder = "0";
    //   const send2 = {holder};
    //   axios.post('http://127.0.0.1:8000/api/log',send2).then(res=>console.log(res.data))
    // }
  }
  refresh2(){
    this.setState({refresh:!this.state.refresh})
